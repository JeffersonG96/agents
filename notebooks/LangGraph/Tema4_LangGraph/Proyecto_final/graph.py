...
#definir la plantilla del estado 
...
class HelpdeskGraph:
    ...
    def procesar_rag(self, state):
        """Buscar el contexto de la consulta utilizando el sistema RAG"""
    ...
    def clasificar_con_contexto(self, state):
        ...
        - AUTOMATICO: Si la información de la BD responde completamente la consulta, tiene buena confianza (>0.6), y es un tema estándar/procedimiento conocido
        
        - ESCALADO: Si la información es insuficiente, confianza baja, problema complejo/único, requiere acceso a sistemas internos, o involucra decisiones de negocio
        ...
        try: 
            ...
            if "automatico" in content or "automático" in content:
                ...
                categoria = "automático" if confianza >= 0.60 else "escalado"
            
            return {
                "categoria": categoria,
                "historial": [
                    f"Clasificación con contexto: {categoria}",
                    f"justificacion: {response.content}"
                ]
            }
    ...
    #preparar escalado para huamano 
    def preparar_escalado(self, sstate):
        """Preparar el escalado a un humano"""
        return {
    ...
    def generar_respuesta_final(self, state):
        """Generar la respuesta final del sistema al ticket del usuario"""
        if state.get("respuesto_final"):
            return {
                "historial":["Respuesta final proporcionada por agente humano"]
            }

        #si no hay respusta final, hace con IA (usa la resuesta del sistema RAG)
    ...
    def decidir_desde_clasificacion(self,state):
        """Decidir hacia donde ir despues de la clasificación concontexto RAG"""
